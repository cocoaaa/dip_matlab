...
%         figure;
%         histogram(diff_im); title('diff_im (double type)');
%         disp('max: '); max(max(diff_im))
%         disp('min: '); min(min(diff_im))
        % Show result at each iteration
%         d=linspace(min(diff_im(:)),max(diff_im(:)),256);
%         unit8_diff_im = uint8(arrayfun(@(x) find(abs(d(:)-x)==min(abs(d(:)-x))),diff_im));
%         imwrite(unit8_diff_im, sprintf('../figs/iter%d.png',t)); 
%         figure;
%         imshow(unit8_diff_im);
end